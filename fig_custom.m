function h = fig_custom(varargin)
% fig_custom - Creates a figure with a desired size, no white-space, and several other options.
%
%       All Matlab figure options are accepted. 
%       fig_custom-specific options of the form fig_custom('PropertyName',propertyvalue,...) 
...
%                      Default is 'off' 
%
%   fig_custom(H) makes H the current figure. 
%   If figure H does not exist, and H is an integer, a new figure is created with
%   handle H.
%
%   fig_custom(H,...) applies the properties to the figure H.
%
%   h = fig_custom(...) returns the handle to the figure created by fig_custom.
%
%   EXAMPLE:
%      h = fig_custom('units','inches','width',7,'height',2,'font','Helvetica','fontsize',16)
%
